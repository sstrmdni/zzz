            ...
            <div class="card mb-4">
                ...
                <div class="row">
                    <div class="text-start ms-4">
                        ...
                        <button class="fa fa-minus text-lg icon icon-shape bg-gradient-danger shadow-success text-center rounded-circle bstn-icon" id="dropRowsBtn" aria-hidden="true"></button>
                    </div>
                </div>
                <form id="contact-form" action="?url=checkprepare" method="post" role="form" enctype="multipart/form-data" autocomplete="off">
                            ...
                            <table class="table align-items-center justify-content-center mb-0 parentTable">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">Worker</th>
                                    </tr>
                                </thead>
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        const dropRowsBtn = document.getElementById("dropRowsBtn");
        ...
        addRowsBtn.addEventListener("click", function(event) {
            ...
            const newRow = `
                <tr>
                    ...
                    <td>
                        <select name='iduser[${rowCount}]' class="form-control">
                            <option value="--">--</option>
                            <?php
                            $selectuser = mysqli_query($conn, "SELECT iduser, nama_user FROM login WHERE role='preparetoko' AND status != 'out'");
                            while ($datauser = mysqli_fetch_array($selectuser)) {
                                echo "<option value=\"" . $datauser['iduser'] . "\">" . $datauser['nama_user'] . "</option>";
                            }
                            ?>
                        </select>
                    </td>
